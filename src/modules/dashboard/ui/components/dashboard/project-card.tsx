...
export const ProjectCard = ({ project, userId }: ProjectCardProps) => {
    ...
    // Owner ile advisor aynı mı? (owner aynı zamanda danışmansa UI farklılaşsın)
    const isOwnerAlsoAdvisor = !!project.advisor && project.owner.id === project.advisor.id;

    // Koşula göre kullanılacak sınıflar (normal: indigo, çakışma: rose)
    const cardBorderClass = isOwnerAlsoAdvisor ? "border-l-4 border-l-rose-700" : "border-l-4 border-l-indigo-500";
    const competitionBadgeClass = isOwnerAlsoAdvisor
        ? "text-rose-700 border-rose-200 bg-rose-500/10 text-xs font-normal"
        : "text-xs font-normal group-hover:border-primary/30 transition-colors";
    ...
    return (
        <Card className={`transition-colors group hover:shadow-md ${cardBorderClass}`}>
                ...
                <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 justify-between items-start lg:items-center">
                    <div className="space-y-3 w-full">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                            <Badge variant="indigo" className={competitionBadgeClass}>
                                {project.competition}
                            </Badge>
                        </div>
